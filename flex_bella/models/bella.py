...
class BellaQuestion(models.Model):
    ...
    trigger_action_id = fields.Many2one(
        comodel_name="bella.action", string="Action that triggers this question"
    )
...
class BellaAnswer(models.Model):
    ...
    follow_up_question_id = fields.Many2one(
        comodel_name="bella.question", string="Follow-up Question"
    )
